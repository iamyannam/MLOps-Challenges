...
1.   When JupyterLab is started, the Jupyter UI button at the top of the lab must open the notebook interface.

2.   For this to work, the running server must meet the following requirements:
...
3.   Start JupyterLab from the virtual environment using the corrected configuration:
     ```
     source /root/code/ml-env/bin/activate
     jupyter lab --config=/root/code/jupyter_lab_config.py --allow-root --no-browser &
     ```

4.   Make sure JupyterLab is running before using the button at the top of the lab.


jupyter_lab_configure.py -
```
